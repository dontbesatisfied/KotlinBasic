...
fun main() {
    ...
    조건문에는 if, when 이 있으며
    조건문의 조건을 판단하기 위해서 비교연산자와 논리연산자를 사용한다
     */

    /*
    if는 if, else if , else 를 조합하여 하나의 if 절이 완성된다
     */
    ...
    /**
     * range를 사용하면 좀더 편리하며 가독성 높게 사용할수 있다. range는 if절 뿐만아니라 다양한곳에서 활용된다
     * 1..10 은 1부터 10까지의 값을 담는
     */
    var range = 1..10
    ...
    화살표 연산자와 else를 조합하여 when절을 완성한다
     */
    ...
    /*
    특정 값을 비교하는데 결과처리가 동일하다면 ,로 구분해서 한번에 비교할 수 있다
    범위값을 가진다면 range를 이용해서도 가능하다
    */
    ...
    /**
     * when절에서 입력 값. 즉, 파라미터를 쓰지 않는다면 if 문처럼 사용할수 있다
     */
