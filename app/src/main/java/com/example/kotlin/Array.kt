...
fun main() {
    /**
     * 반복문을 배우기 앞서 먼저 배열과 컬렉션을 알아야한다
     * 앞서 배운 타입은 모두 하나의 변수에 하나의 값만 저장하도록 되어있지만
     * 개발을 하다보면 하나의 변수에 여러개의 값을 저장해야 할 상황이 있습니다.
     */

    /**
     * 배열의 경우 여러 값을 담을 수 있지만 먼저 공간의 크기를 결정해두고 써야한다.
     * 크기가 정해지면 중간에 추가 및 삭제를 할수 없다.
     * 기본 타입 뒤에 Array를 붙여서 만든다
     */
     ...
     * 컴퓨터언어에서 index(순서)는 0부터 시작을 한다.
     * 그래서 intArray로 예를들면 0부터 시작해서 9에서 끝나는것을 알 수 있다.
     */
    println("Array size : ${intArray.size}")
    println("Last index : ${intArray.lastIndex}")

    /**
     * 공간을 정의해서 array를 선언하면 기본값으로 자동으로 채워져있다
     * [index]를 이용하여 값을 읽거나 수정할수 있지만
     * 정의한 공간을 넘어갈 경우 에러가 발생한다
     */
...
//    intArray[10] = 1 // Error : Exception in thread "main" java.lang.ArrayIndexOutOfBoundsException: 10
    ...
    /**
     * String은 기본 타입이 아니므로 StringArray는 없지만 arrayOf함수를 이용하여 가능하다
     * arrayOf 함수를 사용하면 값과 공간이 동시에 정의할 수 있다.
     */
    ...
    /**
     * 정리하면 array는 한 변수 안에 여러값을 저장하는 공간의 제약이 있고 값의 수정이 자유롭운 타입이다
     * 다음장에서 컬렉션을 알아본다
     */
