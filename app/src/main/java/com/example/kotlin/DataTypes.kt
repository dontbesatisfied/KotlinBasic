...
fun main() {
    ...
    /*
    변수와 상수 앞으로는 그냥 특정하여 상수를 말하지 않는 이상 변수라고 통일해서 이야기하겠다
    변수는 컴퓨터의 메모리에 저장이되는데 이 변수에게 할당할 메모리 크기를 알아야하므로 자료형을 이용하여 정의한다
...
//    근데 왜 아까는 이런것들을 선언안했을까?
    var age = 28
//    자동으로 타입추론이 일어나는것을 확인 할 수가 있다. 말그대로 추론이므로 예기치않은 작동이 일어날수 있으므로 명시적으로 선언해서 쓰는것이 좋을때가 있다 Long, short, byte
